...
static nrf_pwm_values_common_t pwm_sequence_values[NEOPIXEL_MAX_CHAINS * NEOPIXEL_BYTES];
...
void neopixel_write(uint32_t *colors, uint32_t leds_count) {
    ...
    for (uint32_t i=0; i<leds_count; i++) {
        ...
        for (uint8_t j=0; j<NEOPIXEL_BYTES; ++j) {
    ...
    nrf_pwm_sequence_t const seq0 =
        {
            ...
            .length = NEOPIXEL_BYTES * leds_count,
            .repeats = 0,
            .end_delay = 4000  };
